...
const CardDistribution_H =()=> {
    ...
    const distributeNumbers = () => {
        socket.emit('updatePublicCards',roomname.Roomname,[]);
        const numbersArray = Array.from({ length: 52 }, (_, i) => i + 1);
        socket.emit('distribute', roomname.Roomname, numbersArray);
        console.log("distribute-array: ",numbersArray)
    };
    ...
    const handleDropItem = async (itemIndex, targetBoxIndex, sourceBoxIndex) => {
        ...
        if(sourceBoxIndex>1){
            setBoxes((prevBoxes)=>{
                const updatedBoxes = [...prevBoxes];
                movedItem = updatedBoxes[sourceBoxIndex-2].splice(itemIndex,1);
                return updatedBoxes;
            });
        }
        ...
        if(targetBoxIndex>1){
            setBoxes((prevBoxes)=>{
                const updatedBoxes = [...prevBoxes];
                updatedBoxes[targetBoxIndex-2].push(movedItem);
                return updatedBoxes;
            })
        }
    };
    ...
    useEffect(()=>{
        socket.on('numbers', (numbers) => {
            ...
            //     updatedBoxes[0] = [...numbers.slice(0,26)];
            // })
            let pubCards = numbers.map(num=>images[num]);
            // socket.emit('updatePublicCards',roomname.Roomname,pubCards);
            // setPublicCards(numbers.map(num=>images[num]))
        });

        socket.on('activeSocketsInRoom',(rs)=>{
            setRoomSockets(rs);
            setBoxes(Array.from({ length: rs.length }, () => []));
            splitCircularList(rs,rs.indexOf(socket.id));
            console.log(rs);
        });

        
        // socket.emit('updatePublicCards',roomname.Roomname,publicCards);

        socket.on('capturePublicCards',(pubCards)=>{
            console.log(":",areListsEqual(pubCards,publicCards));
            // if(areListsEqual(pubCards,publicCards) == false){
    ...
    return (
        <Container sx={{
            ...
            <Box sx={{
                ...
                <Button variant="contained" onClick={distributeNumbers} sx={{
                    backgroundColor:"#cccccc", color:"black", fontWeight:'400',
                    "&:hover":{color:'white', fontWeight:'600'}, //marginLeft:"29%"
                }}>Distribute</Button>
                <Button variant="contained" onClick={LeaveRoom} sx={{
                    backgroundColor:"white", color:"black", //marginLeft:"38%",
            ...
            <Box sx={{
                    ...
                    {splittedRoomSockets[0].map((socketID, index) => (
                        <Box key={index} sx={{
                            ...
                            <Box sx={{
                                ...
                                <img src={ProfileImages[1+splittedRoomSockets[1].length+index]} width={"100%"}/>
                ...
                <Box sx={{
                    ...
                    {splittedRoomSockets[1].map((socketID, index) => (
                        <Box key={index} sx={{
                            ...
                            {/* <img src={ProfileImages[roomSockets.indexOf(socket.id)+splittedRoomSockets[1].length-index]} width={"20%"}/> */}
                            <Box sx={{
                                display:'flex', flexDirection:'column',justifyContent:"center", alignItems:"center",
                                width:"20%",
                            }}>
                                <img src={ProfileImages[roomSockets.indexOf(socket.id)+splittedRoomSockets[1].length-index]} width={"100%"}/>
                                {/* <p>smu[smu.indexOf(socketID)+1]</p> */}
                                <Typography varient="h6" sx={{color:"white"}}>{roomSockets.indexOf(socketID)}</Typography>
                            </Box>
                        </Box>
                ...
                <Box sx={{
                    ...
                    <Box key={index} sx={{
                        marginLeft:"5px", marginRight:"5px", height:'60px'
                    }}>
...
export default CardDistribution_H;